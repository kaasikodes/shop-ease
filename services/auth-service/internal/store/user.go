...
func (u *UserStore) RemoveMultipleUsers(ctx context.Context, tx *sql.Tx, emails []string) error {
	if len(emails) == 0 {
...
func (u *UserStore) Create(ctx context.Context, tx *sql.Tx, user *User, role *UserRole) error {
	// Queries
	...
	// Insert user
	result, err := tx.ExecContext(ctx, queryUser, user.Name, user.Email, user.Password.hash)
	if err != nil {
...
// Verifying a user
func (u *UserStore) Verify(ctx context.Context, tx *sql.Tx, user *User) error {
...
func (u *UserStore) AssignRole(ctx context.Context, userId int, roleId DefaultRoleID) (*UserRole, error){
	query := `
...
func (u *UserStore) ActivateOrDeactivateRole(ctx context.Context, userId int, roleId DefaultRoleID, isActive bool) (*UserRole, error){
	query := `
		UPDATE userRoles
		SET isActive = $3
...
func (u *UserStore) GetByEmailOrId(ctx context.Context, user *User) ( *User, error ){
...
func (u *UserStore) Get(ctx context.Context, pagination PaginationPayload, filter UserFilterQuery) ([]User, int, error) { 
	var total int
	var users []User
